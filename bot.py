...
# ==================== الإعدادات الأساسية ====================
API_TOKEN = "8350116285:AAHJbi3j31lBy8OoJBTJjSyOR2S3BMXygAg"
ADMIN_ID = 7947679527
DEVELOPER = "@ggzh9"
...
def init_database():
    ...
    c.execute('''CREATE TABLE IF NOT EXISTS github_actions (id INTEGER PRIMARY KEY AUTOINCREMENT, action_type TEXT, bot_id TEXT, status TEXT, created_at TEXT, completed_at TEXT)''')
...
def log_github_action(action_type: str, bot_id: str, status: str = "pending"):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('''INSERT INTO github_actions (action_type, bot_id, status, created_at) VALUES (?, ?, ?, ?)''',
                  (action_type, bot_id, status, datetime.now().isoformat()))
        conn.commit()
        conn.close()
        return True
    except:
...
def update_github_action(action_id: int, status: str):
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute('''UPDATE github_actions SET status = ?, completed_at = ? WHERE id = ?''',
...
def upload_bot_to_github(bot_folder: str, bot_name: str, bot_id: str):
    ...
    try:
        ...
        log_github_action("upload", bot_id, "completed")
        ...
        log_github_action("upload", bot_id, "failed")
        return None
...
def check_expired_bots():
    """التحقق من البوتات المنتهية صلاحيتها وإيقافها وحذفها من GitHub"""
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
...
def check_missing_bots():
    """التحقق من البوتات المفقودة وإعادة تشغيلها"""
    try:
        conn = sqlite3.connect(DB_PATH)
        c = conn.cursor()
        c.execute("SELECT bot_id, file_path FROM bots WHERE status = 'running'")
        ...
        for bot in running_bots:
            ...
            if not os.path.exists(file_path):
                # محاولة استعادة من GitHub
                github_path = get_bot_github_path(bot_id)
                if github_path:
                    # إعادة تحميل البوت
                    restore_bot_from_github(bot_id, github_path)
                    logger.info(f"🔄 تم استعادة البوت {bot_id} من GitHub")
                    
                    # تحديث السيرفر
                    trigger_auto_deploy()
                else:
                    update_bot_status(bot_id, "missing")
                    logger.warning(f"⚠️ البوت {bot_id} مفقود")
        
        return len(running_bots)
    except Exception as e:
        logger.error(f"خطأ في التحقق من البوتات المفقودة: {e}")
        return 0
...
def monitor_bots():
    """مراقبة البوتات وإعادة تشغيلها عند التوقف مع تحديث السيرفر"""
    while True:
        try:
            ...
            # التحقق من البوتات المفقودة
            missing = check_missing_bots()
